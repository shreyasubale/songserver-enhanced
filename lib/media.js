...
function Media(name, path) {
    var nameHash = crypto.createHash('md5').update(name).digest('hex');
    this.name = name;
    this.path = path;
    this._info = metaData[nameHash] || Object.create(null);
    this._info.nameHash = fs.existsSync('./public/albumart/' + nameHash + '.jpg') ? nameHash : null;
    this.likes = 0;
    this._likers = null;
    this._disLikers = null;
    this.disLikes = 0;
    this.playCount = 0;
...
Media.prototype = helper.extend(Object.create(null), {
    ...
    toJSON: function () {
        ...
        return {
            ...
            likers: this._likers ? this._likers.toJSON() : [],
            info: this.getInfo(),
            dislikers: this._disLikers ? this._disLikers.toJSON() : []
        };
    }
});
